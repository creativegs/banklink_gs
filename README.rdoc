...
==== SebLV
  ...
  Banklink::SebLV.private_key = File.read("#{Rails.root}/path/to/your_private_key")
  Banklink::SebLV.bank_certificate = File.read("#{Rails.root}/path/to/bank_public_key")
  Banklink::SebLV.production_url = "https://ib.swedbank.lv/banklink"
...
Helper for view:
    ...
    @helper = Banklink::Swedbank.helper(TRANS_ID, YOUR_SENDER_ID, options) # OR Banklink::SebLV.helper(TRANS_ID, YOUR_SENDER_ID, options)

Then create Notification controller for notify. Then you can notify:

    notify = Banklink::Swedbank.notification(request.raw_post) # OR Banklink::SebLV.notification(request.raw_post)
    @text
